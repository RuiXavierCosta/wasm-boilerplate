...
export default React.forwardRef(function CodeExample(
  { title, description, JS, WASM, results, uri },
  ref
) {
  ...
  const getResults = () => {
    ...
    if (js < wasm) {
      const percentage = Math.round((wasm / js) * 100) - 100;
      return (
        <p>
          JavaScript was {percentage}% faster than Web Assembly... UUUUPS that's
          a little akward. 😅
        </p>
      );
    }
    ...
    if (wasm <= js) {
      const percentage = Math.round((js / wasm) * 100) - 100;
    ...
    >
      <div className="code-content px-2 ps-md-3 pe-md-3 py-5 w-100">
        <div className="row mb-3">
          ...
          <button className="btn btn-light w-auto ms-2" onClick={runExamples}>
            Run
          </button>
        </div>
        <div className="row mb-3">
        ...
        <div className="row comparisson mb-1">
          ...
          <div className="wasm col-12 col-md-6">
            ...
            {WASMResults.time && (
              <p>Time for WASM execution: {WASMResults.time}ms</p>
            )}
          </div>
        </div>
        {JSResults.time && WASMResults.time && getResults()}
        <button className="btn btn-light w-auto mb-2" onClick={runExamples}>
          Run Code
        </button>
      </div>
    </section>
